...
for TAR_FILE in $IMAGE_FILES; do
    ...
    # e.g. temp_image_upload/extension_images/extensions/9.0.1-0-24965341/agent-updater/agent-updater.latest.tar
    # becomes extensions/9.0.1-0-24965341/agent-updater/agent-updater:latest
    ...
    # Replace the last dot with a colon to reconstruct the tag using Bash parameter expansion
    IMAGE_NAME_WITHOUT_TAG=${IMAGE_SUB_PATH_NO_EXT%.*}
    IMAGE_TAG=${IMAGE_SUB_PATH_NO_EXT##*.}
    IMAGE_NAME_WITH_TAG="${IMAGE_NAME_WITHOUT_TAG}:${IMAGE_TAG}"
    
    DESTINATION_REPO="$DEST_REGISTRY/${IMAGE_NAME_WITH_TAG%:*}"
    
    echo "Processing: $IMAGE_NAME_WITH_TAG"
